...
@section('header')
    <nav class="navbar">
        <div class="page-header">
            <h1 class="page-title">@if(isset($post)) Modifier le post @else Créer un post @endif</h1>
...
<div class="glass-card">
    <div class="settings-tab-content active" id="tab-profile">
        <div class="settings-section">
            <h3 class="settings-section-title">@if(isset($post)) Détails du post @else Nouveau post @endif</h3>
            
            <form method="POST" action="{{ isset($post) ? route('posts.update', $post) : route('posts.store') }}" class="settings-form" enctype="multipart/form-data">
                @csrf
                @if(isset($post))
                    @method('PUT')
                @endif
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group-settings">
                        ...
                        <div class="file-input-wrapper">
                            ...
                            <label for="image" class="btn-file-glass">
                                ...
                                <span id="file-name-text">@if(isset($post)) Changer l'image @else Choisir une image @endif</span>
                            </label>
                        </div>
                        @error('image')
                            <span class="error-message" style="color: var(--danger); font-size: 12px;">{{ $message }}</span>
                    ...
                    <button type="submit" class="btn btn-primary" style="width: auto; min-width: 150px;">
                        @if(isset($post)) Mettre à jour @else Publier @endif
                    </button>
                </div>
            </form>
        </div>
